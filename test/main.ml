...
let tests = [
  ...
  make_test "If with binop, call and break" "{'if': [{'cond': {'binop': ==, 'argl': x, 'argr': 5}, 'then': 'break'}], 'else': {'set': x, 'value': {'binop': +, 'argl': x, 'argr': 1}}}"
  ...
  make_test "Block of statements" "[{'return': 2}, {'return': 3}]" "[return 2;, return 3;];";
  make_test "Block of statements with declare and set" "[{'declare': x, 'value': 2}, {'set': x, 'value': 3}]" "[let x=2;, x=3;];";
